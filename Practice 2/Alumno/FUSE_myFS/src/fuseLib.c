 ...
 * @return 0 on success and <0 on error
 **/
static int my_rmnod(const char *path)
{
    fprintf(stderr, "--->>>my_rmnod: path %s\n", path);
    ...
    // The file exists
    int idxFileinDirectory, idxNodoI;
    if((idxFileinDirectory = findFileByName(&myFileSystem, (char *)path + 1)) == -1)
        return -EEXIST;


    // Update root folder
    idxNodoI = myFileSystem.directory.files[idxFileinDirectory].nodeIdx;
...
struct fuse_operations myFS_operations = {
    ...
    .unlink     = my_rmnod,						// Removes a file
};
